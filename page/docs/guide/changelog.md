...
从 **2.0.0** 起。每个版本默认折叠，点版本号展开。最新：**2.2.5**。

完整发布页：[GitHub](https://github.com/qingyueyin/Pure-music/releases) · [Gitee 镜像](https://gitee.com/qingyueyin/Pure-music)（同步常滞后）

<details>
<summary><strong>2.2.5</strong></summary>

**Last.fm**

- 新增 Last.fm 收听记录提交，支持授权绑定、达标自动提交与离线补交
- 设置页-播放 新增 Last.fm 授权面板，可管理绑定状态与 API 凭证

**歌词**

- 韩文歌词分组判定与可变字重回退，韩文排版更准确
- 优化歌词滚动弹簧动画，更连贯
- 尝试适配SPL格式
- 优化原文/翻译/注音 分组判断
...
- 支持透明背景
- 页面切换动效改用统一常量，动效风格更一致
- 点击顶部歌词栏可展开搜索
- 设置-高级-字体 可独立选择歌词字体，并支持预览与搜索

**播放**

- 保留内嵌封面原始格式，修复封面格式转换导致的显示异常
- 修复 EQ、DSP 与输出模式切换问题

**稳定性**

- 新增用户数据备份与恢复功能，支持导出与导入
- 修复依赖安全与日志安全问题

</details>

<details>
<summary><strong>2.2.4</strong></summary>

**歌词**

- 歌词加载返回来源标记，便于追踪歌词来源
- 修复竖排歌词滚动吸附判定问题
- 行推进定时器限制 seek 误跳，播放更流畅
- 折行按整词推进高亮并统一折行宽度，显示更整齐
- 桌面歌词双行模式与多行模式互斥，设置更清晰

**封面**

- 支持获取在线封面
- 音频封面支持写入标签，可在线搜索替换
- 标签失败时区分封面已写入状态，错误处理更精细

**播放**

- 播放完成处理可测试化与过渡补偿，播放更稳定
- 修复艺术家和文件夹定位按钮失效问题
- 修复艺术家/文件夹页面定位当前播放按钮不生效

**界面**

- 统计页平滑滚动与布局优化，浏览更舒适
- 设置页背景/快捷键/任务栏分组与歌词源 UI 重写，设置更直观
- 详情页定位与播放页布局优化，界面更美观

**背景**

- 新增**测试**背景-律动背景
- 律动背景分层模糊与音频律动平滑，视觉效果更出色
- 移除残留静态封面背景

**重构**

- 快捷键系统重构，系统与应用内分离注册，管理更清晰
...
- 新增演出模式，可从曲库、文件夹、歌手、专辑或歌单中自动编排播放顺序
- 支持调节情绪起伏、压轴时机、顺滑度、演出规模和收尾风格
- 提供情绪曲线和历史演出记录，不修改原歌单

**歌词与桌面歌词**

- 支持多行歌词自动换行，逐字高亮在换行后仍能正常跟随
- 歌词详情页支持编辑内嵌歌词、在线搜索并写入标签或外部 LRC
- 桌面歌词支持多行、竖排、双行和更完整的窗口行为设置
- 修复前奏、间奏、句末高亮和多声部歌词显示问题

**播放与界面**

- 专辑详情页新增顺序播放
- 播放页支持始终显示控件，并可直接查看歌词来源
- 封面加载增加稳定占位，长列表支持拼音首字母搜索和字母索引
- 任务栏播放控制与封面预览支持独立设置

**稳定性**

- 优化列表滚动、封面加载、音频分析和页面动效
- 修复播放队列、进度拖动、页面过渡及歌词同步相关问题
...
- 新增智能切歌：自动根据歌曲内容选择最合适的衔接方式

- 切歌方式可选：无缝衔接、淡入淡出、交叉淡化、智能切歌

- 淡出 / 淡入时长可以分别调节

**界面 / 窗口**

- 新增系统托盘图标，点关闭按钮可以选择最小化到托盘，不退出程序

- 鼠标悬停任务栏图标时，可以预览到当前歌曲封面

- 新增独立主题色：主题色和背景可以分开设置，互不影响

- 支持自定义应用背景图片，可以调节透明度和模糊

- 窗口大小恢复正常范围，不会被意外缩到很小
...
- 选择歌词来源前先确认能否下载，失败会提前提示

- 本地歌词文件解析优化，更多歌词文件能正常显示
...
- 修复曲库索引偶尔损坏的问题，扫描过程更稳定

- 修复文件夹选择偶尔卡住的问题

- 新增错误日志，出问题时可以一键导出日志反馈
