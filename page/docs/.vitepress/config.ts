...
const SITE = 'https://qingyueyin.github.io/Pure-music'
const OG_IMAGE = `${SITE}/logo.png`
const DEFAULT_DESC =
  'Windows 本地音乐播放器。导入文件夹即可播放；支持多格式歌词、10 段 EQ、音调与 WASAPI 独占。'

function canonicalFor(relativePath: string) {
  if (relativePath === 'index.md') return `${SITE}/`
  const withoutExt = relativePath.replace(/\.md$/, '')
  if (withoutExt.endsWith('/index')) {
    return `${SITE}/${withoutExt.slice(0, -'index'.length)}`
  }
  return `${SITE}/${withoutExt}.html`
}

export default defineConfig({
  title: 'Pure Music',
  titleTemplate: ':title · Pure Music',
  description: DEFAULT_DESC,
  base: '/Pure-music/',
  lang: 'zh-CN',
  head: [
    ['link', { rel: 'icon', type: 'image/webp', href: '/Pure-music/logo.webp' }],
    ['link', { rel: 'icon', type: 'image/png', href: '/Pure-music/logo.png' }],
    ['link', { rel: 'apple-touch-icon', href: '/Pure-music/logo.png' }],
    ['link', { rel: 'manifest', href: '/Pure-music/site.webmanifest' }],
    ...
    ['meta', { property: 'og:site_name', content: 'Pure Music' }],
  ...
  transformHead({ pageData }) {
    ...
    const title = isHome
      ? 'Pure Music · Windows 本地音乐播放器'
      : `${pageData.title || 'Pure Music'} · Pure Music`
    const desc = pageData.description || DEFAULT_DESC
    ...
    if (isHome) {
      tags.push([
        ...
        JSON.stringify({
          ...
          url: `${SITE}/`,
          downloadUrl: `${SITE}/download.html`,
          image: OG_IMAGE,
          description: DEFAULT_DESC,
        }),
      ])
