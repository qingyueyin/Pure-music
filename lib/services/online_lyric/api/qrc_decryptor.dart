...
List<List<int>> _keySchedule(Uint8List key, int mode) {
  ...
  final List<int> keyRndShift = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1];
  ...
  for (int i = 0; i < 16; i++) {
    ...
    int togen = (mode == _decrypt) ? (15 - i) : i;
...
Future<String?> qrcDecrypt({required dynamic encryptedQrc, required bool isLocal}) async {
  ...
  if (encryptedQrc is String) {
    ...
    debugPrint('qrcDecrypt: unknown type');
    return null;
  }
  ...
  try {
    ...
    debugPrint('qrcDecrypt failed: $e');
    return null;
  }
}
...
class TripleDesDecryptor {
  static const List<int> _qrcKey = [
    33, 64, 35, 41, 40, 36, 37, 42, 36, 35, 49, 51, 52, 51,
    50, 53, 33, 64, 35, 41, 78, 72, 76, 90, 33, 64, 35, 41
  ];

  static Uint8List decrypt(Uint8List encryptedBytes) {
    final result = Uint8List.fromList(List.from(encryptedBytes));

    for (int i = 0; i < result.length; i++) {
      result[i] ^= _qrcKey[i % _qrcKey.length];
    }
